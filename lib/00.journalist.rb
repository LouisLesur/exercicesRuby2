...
num0 = list.select {|item| item.include?("0")}
num1 = list.select {|item| item.include?("1")}
num2 = list.select {|item| item.include?("2")}
num3 = list.select {|item| item.include?("3")}
num4 = list.select {|item| item.include?("4")}
num5 = list.select {|item| item.include?("5")}
num6 = list.select {|item| item.include?("6")}
num7 = list.select {|item| item.include?("7")}
num8 = list.select {|item| item.include?("8")}
num9 = list.select {|item| item.include?("9")}
num = num1 + num2 + num3 + num4 + num5 + num6 + num7 + num8 + num9
puts num.count
#c'est dégueux mais ça marche

#Q3 check
puts "Combien d'handle contiennent les 4 lettres du prénom 'Aude' à la suite (sans prendre en compte les majuscules) ? :"
min = list.select {|item| item.include?("aude")}
maj = list.select {|item| item.include?("Aude")}
result = min + maj
puts result.count

#Q4
puts "Combien commencent par une majuscule (première lettre juste après le @) ? :"
maju = list.select {|item| item =~ /[A-Z\A]/ }
puts maju.count

#Q5
puts "Combien contiennent une majuscule ? :"
majs = list.select {|item| item =~ /[A-Z\d]/ }
puts majs.count

#Q6 fait
puts "Combien y a-t-il de underscore _ dans tous les pseudos confondus ? :"
underscore = list.select {|item| item.include?("_")}
puts underscore.count


#Q7 A REVOIR
puts "Trie de la liste de handle par ordre alphabétique :"
puts list.sort

#Q8
puts "Quels sont les 50 handles les plus courts de ce array ? :" #raté revus après
puts list.min_by(50){|item| item.length}
